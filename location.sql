...
-- We need to create the table location table in a staging and copy the data from the internal stage to table using copy into command--
...
------- Streaming table - we create the streaming table to capture the any chnages happened to that location table that we creaed in staging layer --- 
...
---Copy command is used to copy the data from internal stage to location table --
...
-- creating streaming object in clen layer to capture the changes in clean location table---- 
...
------ Merge the data from location stream object from stage layer to location table in clean layer --- 
...
USING (
    SELECT 
        ...
        COALESCE(TRY_TO_TIMESTAMP_TZ(CreatedDate, 'YYYY-MM-DD HH24:MI:SS'), CURRENT_TIMESTAMP) AS created_ts,
        TRY_TO_TIMESTAMP_TZ(ModifiedDate, 'YYYY-MM-DD HH24:MI:SS') AS modified_ts,
        _stg_file_name,
        _stg_file_load_ts,
        _stg_file_md5,
        CURRENT_TIMESTAMP AS _copy_data_ts
...
---- Create a table in consumpation layer of diemnsion table ---- 
...
---incremental load and any chnages happened in the data ---
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @stage_sch.csv_stg/delta/location/day-03-duplicate-data.csv t
)
file_format = (format_name = 'stage_sch.csv_file_format')
on_error = abort_statement
