...
-- Step 2; we need to extract the info from variant data type as it has lot of information and we need to analyze them. 
...
--- Above one is simple example that we can extract the data from variant data type. Below one is the final one where we do all the possible checks and query the data -- 
...
Select *from MATCH_DETAIL_CLEAN limit 10;

ALTER TABLE clean.MATCH_DETAIL_CLEAN
ADD CONSTRAINT pk_match_type_number
...
select 
raw.info:match_type_number as match_type_number,
raw.info:players as players, 
raw.info:teams as teams
from 
raw.match_raw_tbl raw
...
Desc table clean.player_clean_tbl;

alter table clean.player_clean_tbl
...
----- Now we need to create delivery table -------------- 
